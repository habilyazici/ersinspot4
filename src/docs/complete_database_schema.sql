...
CREATE TABLE IF NOT EXISTS moving_requests (
  ...
  -- Taşınma Detayları
  moving_type VARCHAR(50) NOT NULL, -- 'house', 'office', 'warehouse', etc.
...
CREATE TABLE IF NOT EXISTS moving_request_items (
  id BIGSERIAL PRIMARY KEY,
  request_id BIGINT NOT NULL REFERENCES moving_requests(id) ON DELETE CASCADE,
...
-- Public read/write policies (auth yapılan backend'de kontrol ediliyor)
CREATE POLICY IF NOT EXISTS "Allow public read access" ON customers FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON customers FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON customers FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON admins FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON admins FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON admins FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON technical_service_requests FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON technical_service_requests FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON technical_service_requests FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON technical_service_photos FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON technical_service_photos FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON technical_service_photos FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON technical_service_updates FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON technical_service_updates FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON technical_service_updates FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON moving_requests FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON moving_requests FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON moving_requests FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON moving_request_items FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON moving_request_items FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON moving_request_items FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON moving_request_photos FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON moving_request_photos FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON moving_request_photos FOR UPDATE USING (true);

CREATE POLICY IF NOT EXISTS "Allow public read access" ON moving_appointments FOR SELECT USING (true);
CREATE POLICY IF NOT EXISTS "Allow public insert access" ON moving_appointments FOR INSERT WITH CHECK (true);
CREATE POLICY IF NOT EXISTS "Allow public update access" ON moving_appointments FOR UPDATE USING (true);
...
/*
HİBRİT VERİ YAKLASIMI:
- Eski veriler KV Store'da saklanmaya devam ediyor (products, orders, sell_requests)
- Yeni özellikler Postgres kullanıyor (technical_service, moving, customers, admins)

TEKNİK SERVİS STATUS DEĞERLERİ:
- reviewing: İlk talep alındığında
- quoted: Admin fiyat teklifi verdi
...
NAKLİYE STATUS DEĞERLERİ:
- reviewing: İlk talep alındığında
- quoted: Admin fiyat teklifi verdi
- accepted: Müşteri onayladı
- rejected: Müşteri reddetti
- scheduled: Randevu planlandı
- in_progress: Taşınma devam ediyor
- completed: Tamamlandı
- cancelled: İptal edildi

PHOTO_TYPE DEĞERLERİ:
- problem: Sorun fotoğrafı (teknik servis)
- before: İşlem öncesi
- after: İşlem sonrası
- general: Genel fotoğraf (nakliye)
- invoice: Fatura/Dekont
*/
